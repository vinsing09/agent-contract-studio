...
const navItems = [
  { label: "Agents", path: "/agents", icon: Box, matchPaths: ["/agents"] },
        ...
        {navItems.map((item) => {
          const active =
            item.label === "Test Cases"
              ? location.pathname.match(/^\/agents\/[^/]+\/test-cases/)
              : item.label === "Agents"
                ? location.pathname === "/agents" || location.pathname === "/agents/new"
                : location.pathname === item.path || location.pathname.startsWith(item.path + "/");
          return (
            <Link
              key={item.label}
              to={item.label === "Test Cases" ? "/agents" : item.path}
